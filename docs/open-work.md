...
5. **Stage C — the trap layer, driven by the port's loud-stop order.**
   ⭐ **37 distinct traps currently execute; the next stop is `GetNewDialog` at `load+$0054`.**
...
8. **Port the standing checks.** `docs/amiga-lessons.md` prescribes counters that "must read 0"
...
10. **One dormant link trap left in the vendored framework.** `Bitmap::patternWithMask()` pulls in
...
11. ⚠ **The quit chord is the BARE left mouse button, not `CTRL`+LMB.** `amiga/run.sh` documents
...
12. **Write `ghidra_scripts/DumpTraps.java`.** The trap map is the abstraction boundary and there is
    no inherited script for it (Revs's `DumpHwAccesses.java` hardcodes BBC I/O ranges and was not
    carried over). → `docs/toolchain.md`.
13. **Fill `ghidra_scripts/entrypoints.csv` from `CODE 0`.** The jump table makes the postmortem's
    §1.1 sweep *enumerable* rather than a search — take the win.
14. **Read the manual / the extras before the binary.** RoF's `docs/manual.md` earned its place.
...
15. **Decode the `VETTE!.Data` record formats.** The *inventory* is done
    ...
    exactly this.
16. **Confirm or kill the `OBJS` two-level-of-detail reading.** The `C`/`S` name pairs
...
17. **Explain the `Communication` segment and `COMM` 0.** 9.1 KB of code in *both* builds plus a
...
18. **Explain `FRED`** — 6.5 KB in both builds, name says nothing. ⭐ New evidence, and it is a
...
19. ⭐ **The two display questions that are still open** — the mode itself is now locked
...
20. ⭐ **Locate the copy-protection check, then patch it out.** Decision locked — patched, not

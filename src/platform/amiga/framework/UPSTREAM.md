...
## ⚠⚠ TWO LATENT LINK TRAPS, inherited and VERIFIED HERE (2026-09-16)

Found by partial-linking the framework on its own (`ld -r` over the ten framework objects, which
does not garbage-collect). Both are **dormant**, and dormant in the same way: `--gc-sections` drops
the referencing function while nothing calls it, so the link is clean until the first caller
arrives — at which point it fails, or worse, fails an audit for a reason that looks unrelated.
Recorded here rather than fixed, because fixing either one is a change to vendored code and should
be done when the port actually needs the routine.

1. **`AmigaHardware::isLongFrame()` has an ASSEMBLER bridge with no asm behind it.**
   `AmigaHardware.cpp`'s `#if defined(ASSEMBLER) && !defined(__SASC)` body does
   `jsr _isLongFrame__13AmigaHardwareFv`, and **no `.s` in this framework defines that symbol** —
   `AmigaHardwareAssembler.s` has `getVBR` and `isBlitterBusy` but not `isLongFrame`.
   ⇒ **the first call to `isLongFrame()` is an undefined-symbol link error.** The fix is either to
   add the routine to the `.s` or to move `isLongFrame` out of the bridged set (its C++ body is two
   instructions — a VPOSR bit-15 test — so the bridge buys nothing anyway).

	xdef	_update__4RectFv
	xdef	_unite__4RectFRC4Rect
	xdef	_sqrt__4UtilFUl
	xdef	_ungzip__4UtilFPvPv
...
_update__4RectFv:
	...
	move.w	rectBottomRightX(a0),d0		; width = (uint16_t)(bottomRight.x - topLeft.x + 1);
	sub.w	rectTopLeftX(a0),d0
	addq	#1,d0
	move.w	d0,rectWidth(a0)
	move.w	rectBottomRightY(a0),d0		; height = (uint16_t)(bottomRight.y - topLeft.y + 1);
	sub.w	rectTopLeftY(a0),d0
	addq	#1,d0
	move.w	d0,rectHeight(a0)
	move.w	rectTopLeftX,d0			; center = Point2D((topLeft.x + bottomRight.x) >> 1, (topLeft.y + bottomRight.y) >> 1);
	add.w	rectBottomRightX,d0
	asr.w	#1,d0
	move.w	d0,rectCenterX(a0)
	move.w	rectTopLeftY,d0
	add.w	rectBottomRightY,d0
	asr.w	#1,d0
	move.w	d0,rectCenterY(a0)
	move.w	rectBottomRightX(a0),d2
	cmp.w	rectTopLeftX(a0),d2
	slt	d0
	move.w	rectBottomRightY(a0),d2
	cmp.w	rectTopLeftY(a0),d2
	slt	d1
	move.b	d0,d2				; isEmpty = (bool)((bottomRight.x < topLeft.x || bottomRight.y < topLeft.y) ? true : false);
	or.b	d1,d2
	move.b	d2,rectIsEmpty(a0)
	move.b	d0,d2				; isNull = (bool)((bottomRight.x < topLeft.x && bottomRight.y < topLeft.y) ? true : false);
	and.b	d1,d2
	move.b	d2,rectIsNull(a0)

	move.l	(sp)+,d2
	rts

_unite__4RectFRC4Rect:
	move.w	rectTopLeftX(a1),d0			; topLeft = Point2D(MIN(topLeft.x, rectangle_topLeft.x), MIN(topLeft.y, rectangle_topLeft.y));
	cmp.w	rectTopLeftX(a0),d0
	bge.s	u_topLeftXOk
	move.w	d0,rectTopLeftX(a0)
u_topLeftXOk:
	move.w	rectTopLeftY(a1),d0
	cmp.w	rectTopLeftY(a0),d0
	bge.s	u_topLeftYOk
	move.w	d0,rectTopLeftY(a0)
u_topLeftYOk:

	move.w	rectBottomRightX(a1),d0			; bottomRight = Point2D(MAX(bottomRight.x, rectangle_bottomRight.x), MAX(bottomRight.y, rectangle_bottomRight.y));
	cmp.w	rectBottomRightX(a0),d0
	ble.s	u_bottomRightXOk
	move.w	d0,rectBottomRightX(a0)
u_bottomRightXOk:
	move.w	rectBottomRightY(a1),d0
	cmp.w	rectBottomRightY(a0),d0
	ble.s	u_bottomRightYOk
	move.w	d0,rectBottomRightY(a0)
u_bottomRightYOk:

	bra	_update__4RectFv		; update();
